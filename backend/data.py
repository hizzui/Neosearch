...
def _load_genres_data():
    ...
    json_path = Path(__file__).parent / "data" / "genres.json"
    
    if json_path.exists():
        try:
            with open(json_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
                print(f"✓ Loaded {len(data)} genres from {json_path}")
                return data
        except Exception as e:
            print(f"⚠️  Error loading JSON: {e}, using defaults")
            return DEFAULT_GENRES_DATA
    else:
        print(f"📝 No genres.json found, using default data")
        print(f"   Run 'python data_seed.py' to fetch real Spotify genres")
        return DEFAULT_GENRES_DATA
